...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { StiggInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw StiggInvalidDataException("`$name` is not set")
            is JsonNull -> throw StiggInvalidDataException("`$name` is null")
            else -> throw StiggInvalidDataException("`$name` is invalid, received $this")
        }
