...
private constructor(
    ...
    /**
     * Credit grant object representing allocated credits for a customer. Credit grants cannot be
     * edited after creation via this API — void the grant to stop further consumption from it, then
     * create a new grant with the corrected amount, priority, or expiration.
     *
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [CreditGrantResponse].
         *
    ...
    /** A builder for [CreditGrantResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(creditGrantResponse: CreditGrantResponse) = apply {
            data = creditGrantResponse.data
            additionalProperties = creditGrantResponse.additionalProperties.toMutableMap()
        }

        /**
         * Credit grant object representing allocated credits for a customer. Credit grants cannot
         * be edited after creation via this API — void the grant to stop further consumption from
         * it, then create a new grant with the corrected amount, priority, or expiration.
         */
        ...
        /**
         * Returns an immutable instance of [CreditGrantResponse].
         *
         ...
         */
        fun build(): CreditGrantResponse =
            CreditGrantResponse(checkRequired("data", data), additionalProperties.toMutableMap())
    }
    ...
    private constructor(
         ...
         *
         * Unlike [consumedAmount], this method doesn't throw if the JSON field has an unexpected
         * type.
         */
        @JsonProperty("consumedAmount")
        @ExcludeMissing
        ...
        class Builder internal constructor() {
            ...
            internal fun from(data: Data) = apply {
                ...
                status = data.status
                syncStates = data.syncStates.map { it.toMutableList() }
            ...
            /** The total credits consumed from this grant */
            fun consumedAmount(consumedAmount: Double) =
                consumedAmount(JsonField.of(consumedAmount))
            ...
            /**
             * The effective status of the credit grant. A grant with paymentCollectionMethod NONE
             * or CHARGE becomes ACTIVE (and its credits become usable) as soon as it's created (or
             * as soon as the charge succeeds). A grant with paymentCollectionMethod INVOICE stays
             * PAYMENT_PENDING — its credits are not usable — until the invoice is paid.
             */
            ...
            /** The synchronization states of the entity with external systems */
            fun syncStates(syncStates: List<SyncState>?) =
                syncStates(JsonField.ofNullable(syncStates))

            /** Alias for calling [Builder.syncStates] with `syncStates.orElse(null)`. */
            fun syncStates(syncStates: Optional<List<SyncState>>) =
                syncStates(syncStates.getOrNull())

            /**
             * Sets [Builder.syncStates] to an arbitrary JSON value.
             *
             * You should usually call [Builder.syncStates] with a well-typed `List<SyncState>`
             * value instead. This method is primarily for setting the field to an undocumented or
             * not yet supported value.
             */
        ...
        override fun toString() =
            "Data{id=$id, amount=$amount, comment=$comment, consumedAmount=$consumedAmount, cost=$cost, createdAt=$createdAt, currencyId=$currencyId, customerId=$customerId, displayName=$displayName, effectiveAt=$effectiveAt, expireAt=$expireAt, grantType=$grantType, invoiceId=$invoiceId, latestInvoice=$latestInvoice, metadata=$metadata, paymentCollection=$paymentCollection, priority=$priority, resourceId=$resourceId, sourceType=$sourceType, status=$status, syncStates=$syncStates, updatedAt=$updatedAt, voidedAt=$voidedAt, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is CreditGrantResponse &&
            data == other.data &&
            additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() =
        "CreditGrantResponse{data=$data, additionalProperties=$additionalProperties}"
}
